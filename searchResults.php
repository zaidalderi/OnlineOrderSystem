  ...
  <body>
    ...
    if(isset($_POST['sortSelection'])){
      if($_POST['sortSelection'] === "0"){
        $sql = "SELECT * FROM `menu` WHERE name LIKE ? ORDER BY price DESC";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("s",$searchParam);
        $stmt->execute();
        $result = $stmt->get_result();
        ...
        $sql = "SELECT * FROM `menu` WHERE name LIKE ? ORDER BY price ASC";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("s",$searchParam);
        $stmt->execute();
        $result = $stmt->get_result();
        ...
        $sql = "SELECT * FROM `menu` WHERE name LIKE ? ORDER BY name ASC";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("s",$searchParam);
        $stmt->execute();
        $result = $stmt->get_result();
        ...
        $sql = "SELECT * FROM `menu` WHERE name LIKE ? ORDER BY name DESC";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("s",$searchParam);
        $stmt->execute();
        $result = $stmt->get_result();
      ...
      <div class="album py-5 bg-light">
        <div class="container">
          <div class="card-body d-lg-float-end">
            <h3 class="float-start fw-bold">Results</h3>
