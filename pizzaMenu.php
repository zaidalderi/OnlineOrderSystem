<?php
    ...
    $sql = "SELECT * FROM `menu` WHERE category_id = 2 ORDER BY RAND() LIMIT 9";
    $result = $conn->query($sql);
    if(isset($_POST['sortSelection'])){
      if($_POST['sortSelection'] === "0"){
        $sql = "SELECT * FROM `menu` WHERE category_id = 2 ORDER BY price DESC LIMIT 9";
        $result = $conn->query($sql);
        $selection = $_POST['sortSelection'];
      }else if($_POST['sortSelection'] === "1"){
        $sql = "SELECT * FROM `menu` WHERE category_id = 2 ORDER BY price ASC LIMIT 9";
        $result = $conn->query($sql);
        $selection = $_POST['sortSelection'];
      }else if($_POST['sortSelection'] === "2"){
        $sql = "SELECT * FROM `menu` WHERE category_id = 2 ORDER BY name ASC LIMIT 9";
        $result = $conn->query($sql);
        $selection = $_POST['sortSelection'];
      }else{
        $sql = "SELECT * FROM `menu` WHERE category_id = 2 ORDER BY name DESC LIMIT 9";
        $result = $conn->query($sql);
        $selection = $_POST['sortSelection'];
      }
    }else{
  ...
  <div class="album py-5 bg-light">
    <div class="container">
      <div class="card-body d-lg-float-end">
        <h3 class="float-start fw-bold">Pizzas</h3>
